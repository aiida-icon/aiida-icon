...
class IconCalculation(engine.CalcJob):
    ...
    def define(cls, spec: calcjob.CalcJobProcessSpec) -> None:  # type: ignore[override] # forced by aiida-core
        ...
        spec.input_namespace(
            "models", valid_type=(orm.SinglefileData, orm.RemoteData), required=False
        )
    ...
    def prepare_for_submission(self, folder: folders.Folder) -> datastructures.CalcInfo:
        ...
        master_namelist_data = f90nml.reads(
            self.inputs.master_namelist.get_content(mode="r")
        )
        ...
        if "model_namelist" in self.inputs:
            self.logger.warning(
                "The 'model_namelist' input is deprecated, use 'models.<model-name>' instead!"
            )
...
class IconParser(parser.Parser):
    ...
    def parse(self, **kwargs):  # noqa: ARG002  # kwargs must be there for superclass compatibility
        ...
        restart_indicated = (
            finish_status is FinishStatus.RESTART
            or masternml.read_lrestart_write_last(self.node.inputs.master_namelist)
        )
    ...
    def parse_finish_status(self) -> FinishStatusResult:
        result = FinishStatusResult(
            status=FinishStatus.ERR_MISSING_STATUS, message=None
        )
                ...
                with self.retrieved.open("finish.status", "r") as status_file:
                    ...
                    match out_status:
                        ...
                        case _:
                            ...
                            self.logger.info(
                                "The 'finish.status' file contained an unexpected value."
                            )
            except OSError as err:
                result.status = FinishStatus.ERR_READING_STATUS
                self.logger.warning(str(err))
                self.logger.warning(
                    "The 'finish.status' file was found but could not be read."
                )
        else:
            self.logger.warning(
                "The 'finish.status' file could not be found in the output."
            )
    ...
    def parse_restart_files(self, *, restart_indicated: bool) -> dict[str, RestartResult]:
        ...
        try:
            _ = remote_folder.computer.get_authinfo(
                user=orm.User.collection.get_default()
            )
        except aiidaxc.NotExistent:
            self.logger.info(
                "Can not parse restart file names: not possible to authenticate to the computer"
            )
            return results

        masternml_data = f90nml.read(
            self.node.inputs.master_namelist.get_content(mode="r")
        )
        for model_name, _  in masternml.iter_model_name_filepath(masternml_data):
            results[model_name] = self.parse_restart_files_for_model(model_name=model_name, restart_indicated=restart_indicated)
    ...
    def parse_restart_files_for_model(self: Self, *, model_name: str, restart_indicated: bool) -> RestartResult:
        ...
        try:
            ...
            self.logger.info(
                "Can not parse restart file names, singlefile mode is not supported."
            )
            if restart_indicated:
                result.status = RestartStatus.ERROR
        except exceptions.RemoteModelNamelistInaccessibleError:
            self.logger.warning(
                "Could not parse restart file names from remote model namelists."
            )

        for file_name in files:
            if restart_match := re.match(all_restarts_pattern, file_name):
                result.all_restarts[f"restart_{restart_match['timestamp']}"] = (
                    orm.RemoteData(
    ...
    def parse_output_streams(self) -> dict[str, orm.RemoteData]:
        ...
        for stream_info in modelnml.read_output_stream_infos(modelnml_data):
            ...
            self.logger.info(
                "Registered output stream '%s' -> %s", stream_key, full_output_path
            )
